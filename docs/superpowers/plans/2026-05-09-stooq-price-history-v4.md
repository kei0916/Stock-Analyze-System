# Stooq Price History Ingestion Implementation Plan (v4)
...
**Architecture:** 非同期シーケンシャルダウンロード（1並列・1秒間隔）でstooqサーバー負荷を抑えつつ、10,376社分のCSVをパース・フィルタ・DBにUPSERT。1銘柄ごとにCOMMITしてメモリ節約とエラー分離を両立。SQLite bind parameter上限対策で100行チャンク分割。

**Tech Stack:** Python 3.12, SQLAlchemy (async), httpx, AsyncRateLimiter (既存), csv.DictReader, pytest

**Design Doc:** `docs/superpowers/specs/2026-05-09-stooq-price-history-design-v4.md`
...
| File | Responsibility |
|------|--------------|
| `src/stock_analyze_system/models/price_history.py` | SQLAlchemyモデル `PriceHistory`（created_at修正済み） |
| `src/stock_analyze_system/models/__init__.py` | `PriceHistory` import追加（create_all登録） |
| `src/stock_analyze_system/repositories/price_history.py` | `PriceHistoryRepository`（UPSERT・100行チャンク） |
| `src/stock_analyze_system/ingestion/stooq.py` | `StooqPriceClient`（CSVダウンロード・パース・フィルタ・異常検出・AuthError追加） |
| `src/stock_analyze_system/cli/stooq.py` | `stooq` サブコマンド（download・skip-existing・env fallback・明示エラー分類・auth fail-fast） |
| `src/stock_analyze_system/cli/app.py` | stooqパーサー登録 |
| `src/stock_analyze_system/cli/container.py` | `session`フィールド追加・注入 |
| `tests/unit/ingestion/test_stooq.py` | StooqPriceClientのユニットテスト（正常+異常系・rows順序統一） |
| `tests/unit/repositories/test_price_history_repo.py` | リポジトリテスト（チャンク分割含む・date生成修正） |
| `tests/unit/cli/test_stooq_cli.py` | CLIテスト（env fallback・parser default対応） |
| `tests/integration/test_stooq_download.py` | E2Eテスト（実ネットワーク） |
...
## Task 1: PriceHistory Model + __init__.py Registration
...
```bash
git add src/stock_analyze_system/models/price_history.py src/stock_analyze_system/models/__init__.py tests/unit/models/test_price_history.py
git commit -m "feat(models): add PriceHistory model with __init__ registration"
```

---

## Task 2: PriceHistoryRepository (Chunked Upsert)
...
- [ ] **Step 2: Implement repository with chunking**
...
git commit -m "feat(repositories): add PriceHistoryRepository with chunked bulk upsert"
```

---

## Task 3: StooqPriceClient (User-Agent + Error Detection + AuthError)
...
git commit -m "feat(ingestion): add StooqPriceClient with User-Agent and error detection"
```

---

## Task 4: CLI Command (Session-aware + Skip-existing + Env Fallback + Auth Fail-fast)
...
git commit -m "feat(cli): add stooq download command with skip-existing, env fallback, and auth fail-fast"
```

---

## Task 5: Wire into app.py + container.py
...
- [ ] **Step 2: Modify container.py**

```python
# src/stock_analyze_system/cli/container.py
from sqlalchemy.ext.asyncio import AsyncSession  # ADD if not already imported
...
```bash
git add src/stock_analyze_system/cli/app.py src/stock_analyze_system/cli/container.py
git commit -m "feat(cli): add session to ServiceContainer and wire stooq command"
```
...
- [ ] **Error handling:** All 7 error categories covered (NOT_FOUND, AUTH_ERROR, TIMEOUT, PARSE_ERROR, EMPTY, DB_ERROR, UNKNOWN)
- [ ] **Rate limiting:** 1-second interval enforced
- [ ] **DB safety:** Chunked upsert (100 rows), one commit per ticker, rollback on error
- [ ] **Session access:** `ServiceContainer.session` used with None guard
- [ ] **User-Agent:** Set for terms compliance
- [ ] **Env fallback:** `STOOQ_API_KEY` supported via parser default + handler double-check
- [ ] **Row ordering:** Tests and implementation both use oldest-first (ascending)
- [ ] **Date generation:** Tests use safe `date + timedelta(days=i)` pattern
- [ ] **created_at:** `DateTime` + `server_default=func.now()` (not string literal)
- [ ] **Auth fail-fast:** StooqAuthError exits immediately with non-zero status
- [ ] **Error log directory:** `data/` created with `mkdir(parents=True, exist_ok=True)`

---

## Execution Handoff

**Plan complete and saved to `docs/superpowers/plans/2026-05-09-stooq-price-history-v4.md`.**
