...
def _add_security_headers(response, *, path: str = ""):
...
def _validate_config(config: AppConfig) -> None:
    ...
    if not config.web.password:
        raise ConfigError(
            "web.password is not set (env: WEB_PASSWORD). "
            "Set WEB_PASSWORD in the process environment or "
            "configure web.password in settings.yaml.",
        )
...
def create_app(config: AppConfig | None = None) -> FastAPI:
    ...
    async def add_security_headers(request, call_next):
        ...
        return _add_security_headers(response, path=request.url.path)
