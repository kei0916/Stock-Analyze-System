...
def load_config(config_path: str | Path | None = None) -> AppConfig:
    ...
    if val := os.environ.get("WEB_PASSWORD_HASH"):
        config.web.password_hash = val
    if val := os.environ.get("WEB_SESSION_SECRET"):
        config.web.session_secret = val
    if val := os.environ.get("WEB_ALLOWED_HOSTS"):
        config.web.allowed_hosts = [
            h.strip() for h in val.split(",") if h.strip()
        ]
    if val := os.environ.get("WEB_SESSION_INVALIDATED_AFTER"):
        try:
            config.web.session_issued_after = int(val)
        except ValueError:
            pass
