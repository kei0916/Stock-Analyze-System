...
from stock_analyze_system.cli import (
    auth, company, filings, financial, jobs, quotes, rag, screening, serve, stooq, target,
...
async def main() -> None:
    ...
    # auth hash-password は DB/サービス不要なので config だけ渡す。
    if args.command == "auth":
        result = await args.handler(args, config)
